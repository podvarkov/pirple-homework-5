...
module.exports = describe('Functions failed tests',

  test('Should assoc key to object', done => {
    expect(lib.assoc('baz2', 'bar', {foo: 'bar'})).deepStrictEqual({foo: 'bar', baz: 'bar'})
    expect(lib.assoc('foo', 1, {foo: 'bar'})).deepStrictEqual({foo: 1})
    done()
  }),

  test('Should return property by name', done => {
    expect(lib.prop('bar4', {bar: 'baz', foo: 'bar'})).equal('baz')
    done()
  }),
  ...
  test('Should curry given function', done => {
    ...
    expect(typeof curried).equal('string')
